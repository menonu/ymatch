use axum::{
    extract::{Path, State},
    http::StatusCode,
    Json,
};
use sqlx::{PgPool, Row};
use crate::models::*;

// --- Auth ---
pub async fn guest_login(
    State(pool): State<PgPool>,
    Json(payload): Json<GuestLoginRequest>,
) -> Result<Json<User>, (StatusCode, String)> {
    // 1. Try to find existing user by UUID
    let user = sqlx::query_as::<_, User>("SELECT * FROM users WHERE uuid = $1")
        .bind(&payload.uuid)
        .fetch_optional(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    if let Some(user) = user {
        return Ok(Json(user));
    }

    // 2. Create new Guest User
    let new_username = format!("Guest_{}", &payload.uuid[..8]);
    let user = sqlx::query_as::<_, User>(
        "INSERT INTO users (username, uuid) VALUES ($1, $2) RETURNING *"
    )
    .bind(new_username)
    .bind(&payload.uuid)
    .fetch_one(&pool)
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    Ok(Json(user))
}

pub async fn login(
    State(pool): State<PgPool>,
    Json(payload): Json<LoginRequest>,
) -> Result<Json<User>, (StatusCode, String)> {
    let user = sqlx::query_as::<_, User>("SELECT * FROM users WHERE username = $1")
        .bind(&payload.username)
        .fetch_optional(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    if let Some(user) = user {
        // Simple plaintext check for MVP (matching cleanup logic in signup)
        if user.password_hash.as_deref() == Some(&payload.password) {
            Ok(Json(user))
        } else {
            Err((StatusCode::UNAUTHORIZED, "Invalid credentials".to_string()))
        }
    } else {
        Err((StatusCode::UNAUTHORIZED, "Invalid credentials".to_string()))
    }
}


pub async fn signup(
    State(pool): State<PgPool>,
    Json(payload): Json<CreateUserRequest>,
) -> Result<Json<User>, (StatusCode, String)> {
    // Note: In production, hash password here!
    let user = sqlx::query_as::<_, User>(
        "INSERT INTO users (username, password_hash, device_token) VALUES ($1, $2, $3) RETURNING *"
    )
    .bind(&payload.username)
    .bind(&payload.password) // Plaintext for MVP only
    .bind(&payload.device_token)
    .fetch_one(&pool)
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    Ok(Json(user))
}

pub async fn list_users(State(pool): State<PgPool>) -> Result<Json<Vec<User>>, (StatusCode, String)> {
    let users = sqlx::query_as::<_, User>("SELECT * FROM users")
        .fetch_all(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(users))
}

// --- Events ---
pub async fn list_events(State(pool): State<PgPool>) -> Result<Json<Vec<Event>>, (StatusCode, String)> {
    let events = sqlx::query_as::<_, Event>("SELECT * FROM events ORDER BY created_at DESC")
        .fetch_all(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(events))
}

pub async fn create_event(
    State(pool): State<PgPool>,
    Json(payload): Json<CreateEventRequest>,
) -> Result<Json<Event>, (StatusCode, String)> {
    let event = sqlx::query_as::<_, Event>(
        "INSERT INTO events (name, creator_id) VALUES ($1, $2) RETURNING *"
    )
    .bind(payload.name)
    .bind(payload.creator_id)
    .fetch_one(&pool)
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    Ok(Json(event))
}

// --- Merchandise ---
pub async fn list_merch(
    State(pool): State<PgPool>,
    Path(event_id): Path<i32>,
) -> Result<Json<Vec<Merchandise>>, (StatusCode, String)> {
    let merch = sqlx::query_as::<_, Merchandise>("SELECT * FROM merchandise WHERE event_id = $1")
        .bind(event_id)
        .fetch_all(&pool)
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(merch))
}

pub async fn create_merch(
    State(pool): State<PgPool>,
    Path(event_id): Path<i32>,
    Json(payload): Json<CreateMerchRequest>,
) -> Result<Json<Merchandise>, (StatusCode, String)> {
    let merch = sqlx::query_as::<_, Merchandise>(
        "INSERT INTO merchandise (event_id, name, photo_url) VALUES ($1, $2, $3) RETURNING *"
    )
    .bind(event_id)
    .bind(payload.name)
    .bind(payload.photo_url)
    .fetch_one(&pool)
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    Ok(Json(merch))
}

// --- Inventory ---
pub async fn update_inventory(
    State(pool): State<PgPool>,
    Json(payload): Json<UpdateInventoryRequest>,
) -> Result<Json<InventoryItem>, (StatusCode, String)> {
    // Upsert logic
    let item = sqlx::query_as::<_, InventoryItem>(
        r#"
        INSERT INTO inventory (user_id, merch_id, status, quantity)
        VALUES ($1, $2, $3, $4)
        ON CONFLICT (user_id, merch_id, status)
        DO UPDATE SET quantity = EXCLUDED.quantity, updated_at = NOW()
        RETURNING *
        "#
    )
    .bind(payload.user_id)
    .bind(payload.merch_id)
    .bind(payload.status)
    .bind(payload.quantity)
    .fetch_one(&pool)
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    Ok(Json(item))
}

pub async fn get_user_inventory(
    State(pool): State<PgPool>,
    Path(user_id): Path<i32>,
) -> Result<Json<Vec<InventoryItemWithDetails>>, (StatusCode, String)> {
    let items = sqlx::query_as::<_, InventoryItemWithDetails>(
        r#"
        SELECT
            i.id, i.user_id, i.merch_id, i.status, i.quantity,
            m.name as merch_name, m.photo_url
        FROM inventory i
        JOIN merchandise m ON i.merch_id = m.id
        WHERE i.user_id = $1
        "#
    )
    .bind(user_id)
    .fetch_all(&pool)
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(items))
}

// --- Matches ---
pub async fn trigger_matching(State(pool): State<PgPool>) -> Result<Json<serde_json::Value>, (StatusCode, String)> {
    let count = crate::matching::run_matching_algorithm(&pool).await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e))?;

    Ok(Json(serde_json::json!({ "matches_created": count })))
}

pub async fn list_matches(
    State(pool): State<PgPool>,
    Path(user_id): Path<i32>,
) -> Result<Json<Vec<Match>>, (StatusCode, String)> {
    let matches = sqlx::query_as::<_, Match>(
        "SELECT * FROM matches WHERE user1_id = $1 OR user2_id = $1 ORDER BY created_at DESC"
    )
    .bind(user_id)
    .fetch_all(&pool)
    .await
    .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
    Ok(Json(matches))
}
