...
pub trait MatchingAlgorithm {
    async fn run(&self, pool: &PgPool) -> Result<i32, String>;
}

pub struct MatchingEngine {
    algorithm: Arc<dyn MatchingAlgorithm + Send + Sync>,
}

impl MatchingEngine {
    pub fn new(algorithm: Arc<dyn MatchingAlgorithm + Send + Sync>) -> Self {
        Self { algorithm }
    }

    pub async fn run(&self, pool: &PgPool) -> Result<i32, String> {
        self.algorithm.run(pool).await
    }
}

pub struct StrictGroupMatcher;

#[async_trait]
impl MatchingAlgorithm for StrictGroupMatcher {
    async fn run(&self, pool: &PgPool) -> Result<i32, String> {
