...
pub trait MatchingAlgorithm: Send + Sync {
    /// Executes the matching algorithm and returns the number of matches created.
    async fn execute(&self, pool: &PgPool) -> Result<i32, String>;
}

/// The basic matching algorithm currently in use.
pub struct BasicMatchingAlgorithm;

#[async_trait]
impl MatchingAlgorithm for BasicMatchingAlgorithm {
    async fn execute(&self, pool: &PgPool) -> Result<i32, String> {
...
impl MatchingEngine {
    pub fn new(algorithm: Arc<dyn MatchingAlgorithm>) -> Self {
        Self { algorithm }
    }

    pub async fn run_matching(&self, pool: &PgPool) -> Result<i32, String> {
        self.algorithm.execute(pool).await
    }
}

// Deprecated: use MatchingEngine instead. Kept for backwards compatibility if needed.
pub async fn run_matching_algorithm(pool: &PgPool) -> Result<i32, String> {
    let engine = MatchingEngine::new(Arc::new(BasicMatchingAlgorithm));
    engine.run_matching(pool).await
}
