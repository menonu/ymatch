...
//!
//! [`MessageRepository`] owns the `messages` table operations. Powers the
//! chat endpoints `handlers::messages::list_messages` and
//! `handlers::messages::send_message`.
//!
//! Phase B-3 of #191: migrated from the previous
//! `trait MessageRepository + PgMessageRepository` two-type pattern to a
...
impl MessageRepository {
    ...
    pub async fn list_for_match(&self, match_id: i32) -> Result<Vec<Message>, AppError> {
        let rows = sqlx::query(
