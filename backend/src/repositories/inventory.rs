...
impl InventoryRepository {
    ...
    pub async fn upsert(
        ...
        Ok(InventoryItem {
            ...
            photo_url: None,
            group_name: None,
            // No joined event data on the write-echo path (#173 item #5);
            // the frontend re-fetches via list_for_user before display.
            event_name: None,
        })
    }
    ...
    /// `event_name` (#322 context rendering).
    pub async fn list_for_user(&self, user_id: i32) -> Result<Vec<InventoryItem>, AppError> {
        ...
        Ok(rows
            ...
            .map(|row| InventoryItem {
                ...
                // events.name is NOT NULL; the JOIN guarantees a parent event.
