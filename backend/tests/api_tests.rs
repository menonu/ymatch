...
async fn put_json(pool: &PgPool, uri: &str, body: &str) -> axum::response::Response {
    let app = backend::routes::create_router(pool.clone(), test_storage());
    app.oneshot(
        Request::builder()
            .method("PUT")
...
async fn get_request(pool: &PgPool, uri: &str) -> axum::response::Response {
    let app = backend::routes::create_router(pool.clone(), test_storage());
    app.oneshot(
        Request::builder()
            .method("GET")
...
async fn delete_request(pool: &PgPool, uri: &str) -> axum::response::Response {
    let app = backend::routes::create_router(pool.clone(), test_storage());
    app.oneshot(
        Request::builder()
            .method("DELETE")
...
// --- Event-member API (ADR 0004 §5, #228 PR3b) ---------------------------
//
// POST   /api/v1/events/:id/members/:target_id?user_id=<caller>  # assign editor
// DELETE /api/v1/events/:id/members/:target_id?user_id=<caller>  # revoke editor
// GET    /api/v1/events/:id/members?user_id=<caller>             # list members
//
// Guarded by `event.member.manage` (event creator) + admin superuser bypass;
// no `*.any` override, so a global moderator cannot manage an event's members.

/// Count a user's event-scoped role rows for `event_id` (any role). Used to
/// assert assign/revoke landed in `user_roles`.
async fn event_role_count(pool: &PgPool, user_id: i64, event_id: i64) -> i64 {
    sqlx::query_scalar(
...
async fn has_event_role(pool: &PgPool, user_id: i64, event_id: i64, role_name: &str) -> bool {
