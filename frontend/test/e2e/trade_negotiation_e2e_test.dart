...
Future<int> _applyInventory(_Api a, int matchId, int userId) => _postStatus(
  a,
...
void main() {
  test(
    'end-to-end negotiation: open → rejections → counter to balance → accept (#297)',
    () async {
      final a = _newApi();
      expect(
        await _waitForBackend(a.api),
        isTrue,
        reason: 'Backend not reachable at $_baseUrl',
      );
  ...
  test(
    'counter-offer partial update: qty 0 removes a leg, unspecified legs persist (#297)',
    () async {
      final a = _newApi();
      ...
      expect(match['offeredBy'], m.u2);
      expect(
        _giveTotal(match, m.u1),
        1,
        reason: 'u1 give must persist (partial update)',
      );
      expect(
        _giveTotal(match, m.u2),
        0,
        reason: 'cardB leg must be removed by qty 0',
      );
      expect(_isBalanced(match, m.u1, m.u2), isFalse);
  ...
  test(
    'accept re-validates the full leg set against current want (#297 cap-at-accept)',
    () async {
      final a = _newApi();
      ...
      expect(
        await _setStatus(a, m.id, m.u2, 'ACCEPTED'),
        400,
        reason:
            'balanced but over-cap after WANT lowered must be rejected at accept',
      );
  ...
  test(
    'non-participant is forbidden from proposing and accepting (#297 authz)',
    () async {
      final a = _newApi();
  ...
  test(
    'full happy path: open → accept → complete → apply inventory for both sides (#297)',
    () async {
      final a = _newApi();
