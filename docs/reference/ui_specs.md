...
- **Match Card**: User avatar, username, status chip, **event label** on the header (`Event: {name}` / `イベント : {name}`; #534), **local match datetime** (`created_at`, latest-first within each tab; #476), chat/message action, selected items (Give/Receive) with the **group name on the left of each give/receive row** (shown twice; #534 / #466 display name when set), potential items
...
- **Community** (`CommunityCard`, #570 / #572): sits **next to** the Settings card on wide viewports (≥840px) and stacks below it on narrower widths. Icons only (tooltips for a11y). Destinations are baked in at frontend build from GitHub Secrets (`X_PROFILE_URL`, `DISCORD_INVITE_URL`); an icon is omitted when its URL is unset or not `https`.
