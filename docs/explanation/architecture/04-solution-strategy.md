...
**Known exceptions on `main`:** event create/transfer still open transactions
in handlers (until EventService lands); the background **matching job** also
issues raw SQL rather than going through repositories. Treat those as
follow-up cleanup (#497), not the target. (Global search merch SQL lives in
`MerchandiseRepository::search`; group create/transfer transactions live in
`GroupService`.)
