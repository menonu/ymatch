...
| **Notifications** | Outbound auto-match alerts via **Web Push + VAPID** ([ADR 0015](../adr/0015-web-push-vapid-auto-match.md)); implementation in progress (#179). Until VAPID + subscriptions ship, the send path is a safe log/no-op stub (not drawn on the L3 diagram) |
