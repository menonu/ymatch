...
* [Phase 4 Design](file:///home/menonu/ws/ymatch/docs/explanation/refactoring_phase_4.md): Match/Inventory/Message repository design, N+1 fix (1+4N → 3 queries), state-machine model for the trade lifecycle. Historical (describes the initial `trait + dyn` shape that #191 refined — see below).
* [Backend Refactoring Summary](file:///home/menonu/ws/ymatch/docs/explanation/refactoring_summary.md): Phase 1-5 wrap-up, final architecture, aggregate metrics, follow-up issues. The Repository pattern refactor (#163) is complete as of 2026-06-12.
* [Post-#191 Repository Layer](file:///home/menonu/ws/ymatch/docs/explanation/refactoring_post_191.md): The current shape of the Repository layer — single concrete struct per aggregate, generic `Executor<'c, Database = Postgres>` parameter for transactional methods, no `dyn` indirection, no `RepositoryFuture` boxed future. The PR series #192 - #210 (issue #191) refactored the #163-era `trait + Pg*Repository + Arc<dyn ...>` shape into this form.
